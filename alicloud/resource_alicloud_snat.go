...
func resourceAliyunSnatEntryCreate(d *schema.ResourceData, meta interface{}) error {
	...
	println("snat_table_id: %#v", d.Get("snat_table_id").(string))
	println("source_vswitch_id: %#v", d.Get("source_vswitch_id").(string))
	println("snat_ip: %#v", d.Get("snat_ip").(string))
	...
	println("resp.SnatEntryId: %#v", resp.SnatEntryId)
...
func resourceAliyunSnatEntryRead(d *schema.ResourceData, meta interface{}) error {
	...
	println("snat table id: %s", d.Get("snat_table_id").(string))
	snatEntry, err := client.DescribeSnatEntry(d.Get("snat_table_id").(string))
	...
	println("snatEntry.SnatTableId: %s", snatEntry.SnatTableId)
	println("snatEntry.SourceVSwitchId: %s", snatEntry.SourceVSwitchId)
	println("snatEntry.SnatIp: %s", snatEntry.SnatIp)
