...
* `bucket_name` - (Required) Enter the name of the bucket that contains the objects to find.
* `key_regex` - (Optional) Filter results by key with a regex string.
...
* `objects` - A list of bucket objects. Each element contains the following attributes:
  ...
  * `content_type` - Standard MIME type describing the format of the object data, for example, "application/octet-stream".
