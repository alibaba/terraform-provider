...
This data source provides a list of RDS instances under an Alibaba Cloud account.
...
* `engine` - (Optional) Filter by database type. Options are `MySQL`, `SQLServer`, `PostgreSQL` and `PPAS`. If no value is specified, all types are returned.
* `status` - (Optional) Filter by the status of the instance.
* `db_type` - (Optional) Filter by db type. Values are `Primary` for primary instance, `ReadOnly` for read-only instance, `Guard` for disaster recovery instance, and `Temp` for temporary instance.
* `vpc_id` - (Optional) Used to retrieve instances that belong to specified VPC.
* `vswitch_id` - (Optional) Used to retrieve instances that belong to specified `vswitch` resources.
...
* `instances` - A list of RDS instances. Each element contains the following attributes:
  ...
  * `engine` - Database type. Possible options: `MySQL`, `SQLServer`, `PostgreSQL` and `PPAS`. If no value is specified, all types are returned.
